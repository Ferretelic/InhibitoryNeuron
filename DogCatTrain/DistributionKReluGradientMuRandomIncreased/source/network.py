...
class DistributionConvolutionModelKReluGradientMuRandom(nn.Module):
  def __init__(self, image_size):
    ...
    # Conv1
    self.conv1_1 = DistributionConvolution2DGradientKReluMuRandom(filters=32, kernel_size=(3, 3), input_size=(3,) + image_size, padding=1)
    ...
    ## Conv4
    self.conv4_1 = DistributionConvolution2DGradientKReluMuRandom(filters=128, kernel_size=(2, 2), input_size=conv3_output, padding=1)
    self.batch_normalization4_1 = nn.BatchNorm2d(self.conv4_1.output_size[0])
    self.conv4_2 = DistributionConvolution2DGradientKReluMuRandom(filters=128, kernel_size=(2, 2), input_size=self.conv4_1.output_size, padding=1)
    self.batch_normalization4_2 = nn.BatchNorm2d(self.conv4_2.output_size[0])
    conv4_output = self.get_pool_output(self.conv4_2)

    # Linear
    self.linear_input = conv4_output[0] * conv4_output[1] * conv4_output[2]
  ...
  def forward(self, x):
    ...
    x = self.batch_normalization4_1(self.conv4_1.forward(x))
    x = self.batch_normalization4_2(self.conv4_2.forward(x))
    x = self.drop_out(self.pool(x))
