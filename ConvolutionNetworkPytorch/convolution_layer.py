...
class DistributionConvolution2DNoGrad(nn.Module):
...
class DistributionConvolution2DGrad(nn.Module):
  def __init__(self, filters, kernel_size, input_size, padding=0, stride=1, activation=torch.sigmoid):
    super().__init__()
    ...
    self.distribution = nn.Parameter(torch.tensor(get_distribution_3d(self.output_size, 0.1, (0, 0)), dtype=torch.float32))
  ...
  def forward(self, x):
    ...
    output = torch.mul(output, self.distribution)
