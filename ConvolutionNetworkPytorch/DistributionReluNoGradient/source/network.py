...
class DistributionConvolutionModelReluNoGradient(nn.Module):
  def __init__(self):
    ...
    # Conv1
    self.conv1_1 = DistributionConvolution2DNoGradient(filters=32, kernel_size=(3, 3), input_size=(3, 32, 32), padding=1)
    self.batch_normalization1_1 = nn.BatchNorm2d(self.conv1_1.output_size[0])
    self.conv1_2 = DistributionConvolution2DNoGradient(filters=32, kernel_size=(3, 3), input_size=self.conv1_1.output_size, padding=1)
    self.batch_normalization1_2 = nn.BatchNorm2d(self.conv1_2.output_size[0])
    conv1_output = self.get_pool_output(self.conv1_2)

    # Conv2
    self.conv2_1 = DistributionConvolution2DNoGradient(filters=64, kernel_size=(3, 3), input_size=conv1_output, padding=1)
    self.batch_normalization2_1 = nn.BatchNorm2d(self.conv2_1.output_size[0])
    self.conv2_2 = DistributionConvolution2DNoGradient(filters=64, kernel_size=(3, 3), input_size=self.conv2_1.output_size, padding=1)
    ...
    ## Conv3
    self.conv3_1 = DistributionConvolution2DNoGradient(filters=128, kernel_size=(3, 3), input_size=conv2_output, padding=1)
    self.batch_normalization3_1 = nn.BatchNorm2d(self.conv3_1.output_size[0])
    self.conv3_2 = DistributionConvolution2DNoGradient(filters=128, kernel_size=(3, 3), input_size=self.conv3_1.output_size, padding=1)
